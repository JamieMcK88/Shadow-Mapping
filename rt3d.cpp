...
GLuint initShaders(const char *vertFile, const char *fragFile) {
	...
	// initiate shaders including geometry
GLuint initShadersGeo(const char *vertFile, const char *fragFile, const char *geoFile) {
	GLuint p, f, v, g;
	...
	v = glCreateShader(GL_VERTEX_SHADER);
	f = glCreateShader(GL_FRAGMENT_SHADER);
	g = glCreateShader(GL_GEOMETRY_SHADER); //create geometry shader
	// load shaders & get length of each
	GLint vlen;
	GLint flen;
	GLint glen;
